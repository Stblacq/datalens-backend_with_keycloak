...
class KeycloakHeaders(dl_constants_api_constants.DLHeaders):
    SERVICE_ACCESS_TOKEN = "x-dl-service-user-access-token"


@attr.s(frozen=True)
class KeycloakAuthData(dl_api_commons_base_models.AuthData):
    ...
    def get_headers(
        ...
        return {
            KeycloakHeaders.SERVICE_ACCESS_TOKEN: self._service_access_token,
