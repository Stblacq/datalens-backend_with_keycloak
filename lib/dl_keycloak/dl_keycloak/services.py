...
import dl_keycloak.clients as dl_keycloak_clients


LOGGER = logging.getLogger(__name__)


class KeycloakBaseTokenStorage:
    def __init__(
        self,
        token_refresh_timeout: datetime.timedelta = datetime.timedelta(seconds=60),
    ):
        self._token_refresh_timeout = token_refresh_timeout
        self._token: dl_keycloak_clients.Token | None = None
...
class SyncClientProtocol(typing.Protocol):
    def get_token(self) -> dl_keycloak_clients.Token:
        ...


class KeycloakSyncTokenStorage(KeycloakBaseTokenStorage):
    def __init__(
        self,
...
class AsyncClientProtocol(typing.Protocol):
    async def get_token(self) -> dl_keycloak_clients.Token:
        ...


# TODO: add soft refresh timeout to avoid multiple requests on timeout
class KeycloakAsyncTokenStorage(KeycloakBaseTokenStorage):
    def __init__(
        self,
