...
class FlaskMiddleware:
    _client: clients.KeycloakSyncClient
    _token_storage: services.KeycloakSyncTokenStorage

    def set_up(self, app: flask.Flask) -> None:
        app.before_request(self.process)

    def process(self) -> flask.Response | None:
        service_access_token = flask.request.headers.get(middlewares_models.KeycloakHeaders.SERVICE_ACCESS_TOKEN)
